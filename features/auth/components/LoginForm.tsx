...
import { postAuth } from "@/features/auth/api/auth";

async function swrAuthFetcher(
  _key: string,
  { arg }: { arg: { email: string } }
) {
  return postAuth(arg.email);
}
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/10 backdrop-blur-md shadow-xl p-8 text-[--color-foreground]">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold text-white">Product Manager</h1>
      ...
      {debugToken && (
        <div className="mt-4 rounded-md bg-black/30 p-3 text-xs text-white/80 break-all">
          <div className="mb-1 font-medium">Token (debug):</div>
          {debugToken}
        </div>
      )}
